function day19part1(input) {
    const inputLines = input.split('\n');
    let isWorkflowsEnd = false;
    const workflows = {};
    const parts = [];
    ...
    for (let line of inputLines) {
        if (line.length === 0) {
            isWorkflowsEnd = true;
        } else if (isWorkflowsEnd) {
            const [x, m, a, s] = line.substring(1, line.length - 1).split(',');
            const part = {};
            part['x'] = Number(x.substring(2));
            part['m'] = Number(m.substring(2));
            part['a'] = Number(a.substring(2));
            part['s'] = Number(s.substring(2));
            parts.push(part);
        } else {
            let [name, rulesAsString] = line.split('{');
            const rules = rulesAsString.substring(0, rulesAsString.length - 1).split(',');
            workflows[name] = rules;
        }
    }

    let overallAcceptedPartsRating = 0;

    for (let part of parts) {
        let rules = workflows[FIRST_WORKFLOW];
        let partStatus = 'U';

        while (partStatus == 'U') {
            let nextWorkflow = getNextWorkflow(part, rules);
            if (nextWorkflow != 'A' && nextWorkflow != 'R') {
                rules = workflows[nextWorkflow];
            } else {
                partStatus = nextWorkflow;
            }
        }

        if (partStatus == 'A') {
            let partRating = part.x + part.m + part.a + part.s;
            overallAcceptedPartsRating += partRating;
        }
    }

    function getNextWorkflow(part, rules) {
        let nextWorkflowMatch = '';
        for (let i = 0; i < rules.length; i++) {
            ...
            if (ruleSign == '') {
                nextWorkflowMatch = rules[i];
                continue;
            }

            let [categoryName, data] = rules[i].split(ruleSign);
            let [value, nextWorkflow] = data.split(':');

            switch (ruleSign) {
                case '>':
                    if (part[categoryName] > Number(value)) {
                        nextWorkflowMatch = nextWorkflow;
                        isRuleMatched = true;
                    }
                    break;
                case '<':
        ...
        return nextWorkflowMatch;
    }

    console.log('Overall rating of all acceptet parts: ' + overallAcceptedPartsRating);
}

day19part1(`px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
