function day01part2(input) {
    ...
    const checkForNumber = (subString) => {

        const numbers = Object.entries(digitsNames);

        for (let i = 0; i < numbers.length; i++) {
            if (subString.startsWith(numbers[i][0])) {
                return numbers[i][1];
            }
        }
    ...
    const result = rowsData.reduce((acc, curr) => {
        const numsInRow = [];

        for (let i = 0; i < curr.length; i++) {

            const substring = curr.substring(i);

            if (/^\d/.test(substring)) {
                numsInRow.push(substring.charAt(0));
            } else {
                const num = checkForNumber(substring);
        ...
        if (numsInRow.length > 0) {
            acc += Number('' + numsInRow[0] + numsInRow[numsInRow.length - 1]);
        }
...
day01part2(`51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
