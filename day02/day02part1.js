function day02part1(input) {
    ...
    const cubsInBagArr = Object.entries(cubesInBag);
    ...
    rowsData.forEach(row => {
        ...
        const cubesCombinations = cubes.split('; ').map(c => c.split(', '));
        ...
        for (let i = 0; i < cubesCombinations.length; i++) {

            const cubeCombination = {};

            cubesCombinations[i].forEach(c => {
                const [num, color] = c.split(' ')
                cubeCombination[color] = Number(num);
            });

            gameDetails.combinations.push(cubeCombination);
        }
    ...
    const isCombinationsPossible = (combination) => {

        for (let i = 0; i < cubsInBagArr.length; i++) {

            const color = cubsInBagArr[i][0];
            const num = cubsInBagArr[i][1];

            if (combination.hasOwnProperty(color)) {

                if (combination[color] > num) {
                    return false;
                }
            }
    ...
    const result = games.reduce((acc, curr) => {
        ...
        let isImposibleCombination = false;

        for (let i = 0; i < curr.combinations.length; i++) {

            const res = isCombinationsPossible(curr.combinations[i]);

            if (!res) {
                isImposibleCombination = true;
                break;
            }

        }

        if (!isImposibleCombination) {
...
day02part1(`Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`);

day02part1(`Game 1: 4 blue, 4 red, 16 green; 14 green, 5 red; 1 blue, 3 red, 5 green
